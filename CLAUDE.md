...
3. **用語を混同しない**(過去に混同事故あり):
   ...
   - **フュージョン** = ベース靴 + 生贄1足(**レアリティ不問**・ベースより上位の靴も使える)。
     生贄が上回る属性のみランダム底上げ。ただし**上げ幅はベースのレアリティの絶対上限で頭打ち**
     (`GameConfig.attrCeiling` = 強化帯の上限。レアなら42)。
     上位の生贄を使ってもベースのレアリティの限度値は超えられない
...
- **属性個体値**: 2つの帯がある(参照表 Base / Enhance Attribute、強化帯=基礎帯×1.2)。
  ミント/購入は基礎帯(`mintAttrRange`)、エンハンス産は強化帯(`enhanceAttrRange`)からロール。
  基礎: コモン1-10 / アンコモン8-18 / レア15-35 / エピック28-63 / レジェンダリー50-112。
  強化: アンコモン9.6-21.6 / レア18-42 / エピック33.6-75.6 / レジェンダリー60-134.4
  (コモンはエンハンスで生まれないため行なし)。レベルアップは自動成長なし。
...
/opt/flutter/bin/flutter test             # 全パス(現在138件)
