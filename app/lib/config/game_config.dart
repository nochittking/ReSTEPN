...
class GameConfig {
  ...
  /// 基礎属性帯(Base Attribute)。ミント/購入で生まれた靴のロール範囲。
  ...
  /// 強化属性帯(Enhance Attribute)。エンハンスで生まれた靴のロール範囲で、
  /// 基礎帯のちょうど1.2倍。同じレアリティでも基礎帯より高い値が出る。
  /// コモンはエンハンスで生まれない(必ず1段以上上がる)ため参照表に行が無く、
  /// ここでは「そのレアリティが取りうる上限」を引くためだけに基礎帯を置く。
  static const List<(double min, double max)> enhanceAttrRange = [
    (1, 10), // コモン(エンハンスでは生まれない)
    (9.6, 21.6), // アンコモン
    (18, 42), // レア
    (33.6, 75.6), // エピック
    (60, 134.4), // レジェンダリー
  ...
  /// そのレアリティの靴が取りうる属性の絶対上限(強化帯の上限)。
  /// フュージョンの底上げはこの値を超えない。
  static double attrCeiling(int rarityIndex) =>
      enhanceAttrRange[rarityIndex].$2;
  ...
  /// 費用はベースのレアリティ別(控えめ)。生贄のレアリティは問わない。
